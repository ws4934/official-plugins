...
type BackendSummaryReq struct {
	gmeta.Meta `path:"/backend-summary" method:"get" tags:"Dynamic Plugin Demo" summary:"Query the dynamic plugin backend execution summary" dc:"Return the current bridge execution summary for linapro-demo-dynamic when dispatched through the host prefix /x/{pluginId} and this sample's /api/v1 route group, including plugin ID, route information, and current user context." access:"login" permission:"linapro-demo-dynamic:backend:view" operLog:"other"`
}
...
type BackendSummaryRes struct {
	...
	PublicPath    string  `json:"publicPath" dc:"The currently hit host's public routing path" eg:"/x/linapro-demo-dynamic/api/v1/backend-summary"`
